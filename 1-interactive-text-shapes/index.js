...
const draw = SVG().addTo('#drawing').viewbox(-50, 0, 300, 300)
const textSize = 3.5
const rectangleAmount = 10

for (let i = 0; i< rectangleAmount; i++){
	draw.rect(10, 10).draggable().x(i * 10)
}
...
var text = nested.text(customText)
...
const customTextPath = text.path(customPath)
customTextPath.font({ size: textSize, family: 'Verdana' })
...
*/
//Broken code to animate text along a path
//customTextPath.animate(8000, '<>').during(function(pos, morph, eased){
//    var p = text.path().pointAt(eased * length)
//    rect.center(p.x, p.y)
//}).loop(true, true)